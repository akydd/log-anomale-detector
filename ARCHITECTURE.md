...
| `anomaly-detector` | `kinesis:GetRecords`, `kinesis:GetShardIterator`, `kinesis:DescribeStream`, `kinesis:ListShards` | One stream |
...
- **Kinesis stream** — accepts writes only from the CloudWatch Logs service principal (`logs.amazonaws.com`) for the subscription filter
...
- DynamoDB, Kinesis, and CloudWatch Logs are encrypted at rest by default
...
| API Gateway | REST API for dashboard (API key auth) |
